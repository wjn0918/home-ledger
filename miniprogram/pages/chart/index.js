...
Page({
  data: { rows: [], families: [], familyIndex: 0, currentFamilyName: '' },
  ...
  async loadFamiliesAndChart() {
    try {
      ...
      const rows = await request(`/charts/summary?family_id=${data.selectedFamilyId}`)
      this.setData({ rows })
    } catch (e) {
  ...
  async onFamilyChange(e) {
    ...
    const rows = await request(`/charts/summary?family_id=${target.id}`)
    this.setData({ rows })
  }
})
